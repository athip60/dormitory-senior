  ...
  onSubmit() {
    this.dialogService.openDialogConfirm(`ยืนยันการเพิ่มรูปภาพ`, `ยืนยันที่สลิปใบแจ้งชำระเงินหรือไม่?`).afterClosed().subscribe(res => {
      if (res === "true") {
        ...
        this.http.post<any>(`http://188.166.188.246:8080/api/bill/upload-payment/${this.data.data.room_number + '_' + this.data.data.user_in_room + '_' + latest_date}`, formData).subscribe((res) => {
          if (res) {
